...
int readConfig(config* conf){
    ...
    printf("*nick: %s\n", conf->nick);
    printf("*channel: %s\n", conf->channel);
    if(!conf->host || !conf->port || !conf->nick || !conf->channel) {
        printf("didn't get full config\n");
        return 1;
    }
    return 0;
}
...
void processMessage(config conf, char* from, char* where, char* target, char* message){
    if(!strncmp(message, conf.nick, strlen(conf.nick)) && message[strlen(conf.nick)] == ':') {
        char* action = NULL;
        char* args = NULL;
        //Addressed to us (in format 'nick: some message'
        //strip out our name
        message += strlen(conf.nick)+1;
...
int main() {
    ...
    getaddrinfo(conf.host, conf.port, &hints, &res);
            ...
            if ((i > 0 && sbuf[i] == '\n' && sbuf[i - 1] == '\r') || o == 512) {
                ...
                if (!strncmp(buf, "PING", 4)) {
                    ...
                    if (!strncmp(command, "001", 3)) {
                        ...
                        processMessage(conf, user, where, target, message);
                    }
